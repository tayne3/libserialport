...
static FILE* fopen_cloexec_rdonly(const char* pathname) {
    int fd;
    if ((fd = open(pathname, O_RDONLY | O_CLOEXEC)) < 0) return NULL;
    return fdopen(fd, "r");
}
...
SP_PRIV enum sp_return get_port_details(struct sp_port* port) {
    ...
    if (strncmp(port->name, "/dev/", 5)) RETURN_ERROR(SP_ERR_ARG, "Device name not recognized");

    snprintf(link_name, sizeof(link_name), "/sys/class/tty/%s", dev);
    if (lstat(link_name, &statbuf) == -1) RETURN_ERROR(SP_ERR_ARG, "Device not found");
    if (!S_ISLNK(statbuf.st_mode)) snprintf(link_name, sizeof(link_name), "/sys/class/tty/%s/device", dev);
    count = readlink(link_name, file_name, sizeof(file_name));
    if (count <= 0 || count >= (int)(sizeof(file_name) - 1)) RETURN_ERROR(SP_ERR_ARG, "Device not found");
    file_name[count] = 0;
    ...
    if (port->transport == SP_TRANSPORT_USB) {
        for (i = 0; i < 5; i++) {
            ...
            if (!(file = fopen_cloexec_rdonly(file_name))) continue;
            ...
            if (!file || !ptr) port->description = strdup(dev);
            ...
            if (port->usb_serial && strlen(port->usb_serial)) {
                snprintf(description, sizeof(description), "%s - %s", port->description, port->usb_serial);
...
SP_PRIV enum sp_return list_ports(struct sp_port*** list) {
    ...
    if (!(dir = opendir("/sys/class/tty"))) RETURN_FAIL("Could not open /sys/class/tty");

    DEBUG("Iterating over results");
    while ((entry = readdir(dir))) {
        snprintf(buf, sizeof(buf), "/sys/class/tty/%s", entry->d_name);
        if (lstat(buf, &statbuf) == -1) continue;
        if (!S_ISLNK(statbuf.st_mode)) snprintf(buf, sizeof(buf), "/sys/class/tty/%s/device", entry->d_name);
        len = readlink(buf, target, sizeof(target));
        if (len <= 0 || len >= (int)(sizeof(target) - 1)) continue;
        target[len] = 0;
