...
static int strend(const char* str, const char* pattern) {
    ...
    if (slen >= plen) return (!memcmp(pattern, (str + slen - plen), plen));
    return 0;
}
...
static int libusb_query_port(struct libusb20_device* dev, int idx, char** drv_name_str, char** drv_inst_str) {
    ...
    if (!drv_name_str || !drv_inst_str) return -1;
...
static int sysctl_query_dev_drv(const char* drv_name_str, const char* drv_inst_str, char** ttyname,
    ...
    if (!ttyname || !ttyport_cnt) return -1;

    snprintf(sbuf, sizeof(sbuf), "dev.%s.%s.ttyname", drv_name_str, drv_inst_str);
    tbuf_len = sizeof(tbuf) - 1;
    if ((rc = sysctlbyname(sbuf, tbuf, &tbuf_len, NULL, 0)) != 0) return rc;

    tbuf[tbuf_len] = 0;
    *ttyname       = strndup(tbuf, tbuf_len);
    DBG("sysctl: '%s' (%d) (%d): '%.*s'\n", sbuf, rc, (int)tbuf_len, (int)tbuf_len, tbuf);

    snprintf(sbuf, sizeof(sbuf), "dev.%s.%s.ttyports", drv_name_str, drv_inst_str);
    tbuf_len = sizeof(tbuf) - 1;
...
SP_PRIV enum sp_return get_port_details(struct sp_port* port) {
    ...
    while (cua_dev_found == 0) {
        ...
        for (idx = 0; idx <= UINT8_MAX - 1; idx++) {
            ...
            if (drv_name_str) free(drv_name_str);
            if (drv_inst_str) free(drv_inst_str);
            if (cua_dev_found) break; /* Break out of USB device port idx loop. */
        }
    ...
    if (cua_dev_found == 0) DBG("WARN: Found no match '%s' %s'\n", port->name, cua_sfx);
...
SP_PRIV enum sp_return list_ports(struct sp_port*** list) {
    ...
    if (!(dir = opendir("/dev"))) RETURN_FAIL("Could not open dir /dev");
    ...
    while ((entry = readdir(dir))) {
        ...
        if (strncmp(entry->d_name, "cuaU", 4) != 0)
            if (strncmp(entry->d_name, "cuau", 4) != 0)
                if (strncmp(entry->d_name, "cuad", 4) != 0) continue;
        if (strend(entry->d_name, ".init")) continue;
        if (strend(entry->d_name, ".lock")) continue;
        ...
        close(fd);
        if (ret < 0 || cfgetospeed(&tios) <= 0 || cfgetispeed(&tios) <= 0) continue;
