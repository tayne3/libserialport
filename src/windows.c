...
static void enumerate_hub(struct sp_port* port, const char* hub_name, const char* parent_path, DEVINST dev_inst);
...
static char* wc_to_utf8(PWCHAR wc_buffer, ULONG wc_bytes) {
    ...
    if (!(wc_str = malloc((wc_length + 1) * sizeof(WCHAR)))) goto wc_to_utf8_end;
    ...
    if (!(utf8_bytes = WideCharToMultiByte(CP_ACP, WC_NO_BEST_FIT_CHARS, wc_str, -1, NULL, 0, NULL, NULL)))
        goto wc_to_utf8_end;

    /* Allocate UTF-8 output buffer. */
    if (!(utf8_str = malloc(utf8_bytes))) goto wc_to_utf8_end;

    /* Actually converted to UTF-8. */
    if (!WideCharToMultiByte(CP_ACP, WC_NO_BEST_FIT_CHARS, wc_str, -1, utf8_str, utf8_bytes, NULL, NULL)) {
        free(utf8_str);
        utf8_str = NULL;
        goto wc_to_utf8_end;
    }
...
static char* get_root_hub_name(HANDLE host_controller) {
    ...
    if (!DeviceIoControl(host_controller, IOCTL_USB_GET_ROOT_HUB_NAME, 0, 0, &root_hub_name, sizeof(root_hub_name),
                         &size, NULL))
        return NULL;

    /* Allocate wide char root hub name string. */
    size = root_hub_name.ActualLength;
    if (!(root_hub_name_wc = malloc(size))) return NULL;

    /* Actually get the root hub name string. */
    if (!DeviceIoControl(host_controller, IOCTL_USB_GET_ROOT_HUB_NAME, NULL, 0, root_hub_name_wc, size, &size, NULL)) {
...
static char* get_external_hub_name(HANDLE hub, ULONG connection_index) {
    ...
    if (!DeviceIoControl(hub, IOCTL_USB_GET_NODE_CONNECTION_NAME, &ext_hub_name, sizeof(ext_hub_name), &ext_hub_name,
                         sizeof(ext_hub_name), &size, NULL))
        return NULL;

    /* Allocate wide char external hub name string. */
    size = ext_hub_name.ActualLength;
    if (size <= sizeof(ext_hub_name) || !(ext_hub_name_wc = malloc(size))) return NULL;

    /* Get the name of the external hub attached to the specified port. */
    ext_hub_name_wc->ConnectionIndex = connection_index;
    if (!DeviceIoControl(hub, IOCTL_USB_GET_NODE_CONNECTION_NAME, ext_hub_name_wc, size, ext_hub_name_wc, size, &size,
                         NULL)) {
...
static char* get_string_descriptor(HANDLE hub_device, ULONG connection_index, UCHAR descriptor_index) {
    char                    desc_req_buf[sizeof(USB_DESCRIPTOR_REQUEST) + MAXIMUM_USB_STRING_LENGTH] = {0};
    ...
    if (!DeviceIoControl(hub_device, IOCTL_USB_GET_DESCRIPTOR_FROM_NODE_CONNECTION, desc_req, size, desc_req, size,
                         &size, NULL) ||
        size < 2 || desc->bDescriptorType != USB_STRING_DESCRIPTOR_TYPE || desc->bLength != size - sizeof(*desc_req) ||
        desc->bLength % 2)
        return NULL;
...
static void enumerate_hub_ports(struct sp_port* port, HANDLE hub_device, ULONG nb_ports, const char* parent_path,
                                DEVINST dev_inst) {
    ...
    for (index = 1; index <= nb_ports; index++) {
        ...
        if (!(connection_info_ex = malloc(size))) break;

        connection_info_ex->ConnectionIndex = index;
        if (!DeviceIoControl(hub_device, IOCTL_USB_GET_NODE_CONNECTION_INFORMATION_EX, connection_info_ex, size,
            ...
            if (!DeviceIoControl(hub_device, IOCTL_USB_GET_NODE_CONNECTION_INFORMATION, connection_info, size,
        ...
        if (connection_info_ex->DeviceIsHub) {
            ...
            if ((ext_hub_name = get_external_hub_name(hub_device, index))) {
                snprintf(path, sizeof(path), "%s%ld.", parent_path, connection_info_ex->ConnectionIndex);
            ...
            if (connection_info_ex->DeviceDescriptor.iManufacturer)
                port->usb_manufacturer =
                    get_string_descriptor(hub_device, index, connection_info_ex->DeviceDescriptor.iManufacturer);
            if (connection_info_ex->DeviceDescriptor.iProduct)
                port->usb_product =
                    get_string_descriptor(hub_device, index, connection_info_ex->DeviceDescriptor.iProduct);
            if (connection_info_ex->DeviceDescriptor.iSerialNumber) {
                port->usb_serial =
                    get_string_descriptor(hub_device, index, connection_info_ex->DeviceDescriptor.iSerialNumber);
...
static void enumerate_hub(struct sp_port* port, const char* hub_name, const char* parent_path, DEVINST dev_inst) {
    ...
    if (!(device_name = malloc(strlen("\\\\.\\") + strlen(hub_name) + 1))) return;
    strcpy(device_name, "\\\\.\\");
    strcat(device_name, hub_name);
    hub_device = CreateFileA(device_name, GENERIC_WRITE, FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
    free(device_name);
    if (hub_device == INVALID_HANDLE_VALUE) return;

    /* Get the number of ports of the hub. */
    if (DeviceIoControl(hub_device, IOCTL_USB_GET_NODE_INFORMATION, &hub_info, size, &hub_info, size, &size, NULL))
        /* Enumerate the ports of the hub. */
        enumerate_hub_ports(port, hub_device, hub_info.u.HubInformation.HubDescriptor.bNumberOfPorts, parent_path,
                            dev_inst);

    CloseHandle(hub_device);
}

static void enumerate_host_controller(struct sp_port* port, HANDLE host_controller_device, DEVINST dev_inst) {
    char* root_hub_name;
...
static void get_usb_details(struct sp_port* port, DEVINST dev_inst_match) {
    ...
    device_info =
        SetupDiGetClassDevs(&GUID_CLASS_USB_HOST_CONTROLLER, NULL, NULL, DIGCF_PRESENT | DIGCF_DEVICEINTERFACE);
    ...
    for (i = 0; SetupDiEnumDeviceInfo(device_info, i, &device_info_data); i++) {
        ...
        if (!SetupDiEnumDeviceInterfaces(device_info, 0, &GUID_CLASS_USB_HOST_CONTROLLER, i, &device_interface_data))
            continue;

        if (!SetupDiGetDeviceInterfaceDetail(device_info, &device_interface_data, NULL, 0, &size, NULL) &&
            GetLastError() != ERROR_INSUFFICIENT_BUFFER)
            continue;

        if (!(device_detail_data = malloc(size))) continue;
        device_detail_data->cbSize = sizeof(*device_detail_data);
        if (!SetupDiGetDeviceInterfaceDetail(device_info, &device_interface_data, device_detail_data, size, &size,
                                             NULL)) {
        ...
        host_controller_device =
            CreateFile(device_detail_data->DevicePath, GENERIC_WRITE, FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
...
SP_PRIV enum sp_return get_port_details(struct sp_port* port) {
    ...
    if (device_info == INVALID_HANDLE_VALUE) RETURN_FAIL("SetupDiGetClassDevs() failed");
    ...
    for (i = 0; SetupDiEnumDeviceInfo(device_info, i, &device_info_data); i++) {
        ...
        if (strcmp(value, port->name)) continue;
        ...
        while (CM_Get_Parent(&dev_inst, dev_inst, 0) == CR_SUCCESS &&
               (cr = CM_Get_DevNode_Registry_PropertyA(dev_inst, CM_DRP_CLASS, 0, class, &size, 0)) != CR_SUCCESS) {}
        if (cr == CR_SUCCESS) {
            if (!strcmp(class, "USB")) port->transport = SP_TRANSPORT_USB;
        }

        /* Get port description (friendly name). */
        dev_inst = device_info_data.DevInst;
        size     = sizeof(description);
        while ((cr = CM_Get_DevNode_Registry_PropertyA(dev_inst, CM_DRP_FRIENDLYNAME, 0, description, &size, 0)) !=
                   CR_SUCCESS &&
               CM_Get_Parent(&dev_inst, dev_inst, 0) == CR_SUCCESS) {}
        if (cr == CR_SUCCESS) port->description = strdup(description);
        ...
        if (port->transport == SP_TRANSPORT_USB) {
            ...
            do {
                ...
                if (CM_Get_DevNode_Registry_PropertyA(dev_inst, CM_DRP_COMPATIBLEIDS, 0, &compat_ids, &size, 0) ==
                    CR_SUCCESS) {
                    while (*p) {
                        if (!strncmp(p, "USB\\COMPOSITE", 13)) break;
                        p += strlen(p) + 1;
                    }
                    if (*p) continue;
                }
                ...
                if (!strncmp(device_id, "USB\\ROOT", 8)) break;

                /* Prepend the address of current USB layer to the USB path. */
                DWORD address;
                size = sizeof(address);
                if (CM_Get_DevNode_Registry_PropertyA(dev_inst, CM_DRP_ADDRESS, 0, &address, &size, 0) == CR_SUCCESS) {
                    strcpy(tmp, usb_path);
                    snprintf(usb_path, sizeof(usb_path), "%d%s%s", (int)address, *tmp ? "." : "", tmp);
                }
            } while (CM_Get_Parent(&dev_inst, dev_inst, 0) == CR_SUCCESS);
            ...
            handle = CreateFileA(escaped_port_name, GENERIC_READ, 0, 0, OPEN_EXISTING,
...
SP_PRIV enum sp_return list_ports(struct sp_port*** list) {
    ...
    if ((result = RegQueryInfoKey(key, NULL, NULL, NULL, NULL, NULL, NULL, NULL, &max_value_len, &max_data_size, NULL,
                                  NULL)) != ERROR_SUCCESS) {
        SetLastError(result);
    ...
    while (value_len = max_value_len + 1, data_size = max_data_size,
           RegEnumValue(key, index, value, &value_len, NULL, &type, (LPBYTE)data, &data_size) == ERROR_SUCCESS) {
        if (type == REG_SZ) {
