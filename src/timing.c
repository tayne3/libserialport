...
SP_PRIV void time_get(struct time* time) {
    ...
    struct timespec ts;
    if (clock_gettime(CLOCK_MONOTONIC, &ts) == -1) clock_gettime(CLOCK_REALTIME, &ts);
...
SP_PRIV void time_as_timeval(const struct time* time, struct timeval* tv) {
    ...
    tv->tv_usec = (long)((time->ticks % frequency.QuadPart) / (frequency.QuadPart / 1000000));
#else
    *tv = time->tv;
#endif
}
...
SP_PRIV bool timeout_check(struct timeout* timeout) {
    if (!timeout->calls_started) return false;

    if (timeout->ms == 0) return false;
